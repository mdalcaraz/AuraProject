...
void UAuraAttributeSet::Debuff(const FEffectProperties& Props)
{
	...
	if (FGameplayEffectSpec* MutableSpec = new FGameplayEffectSpec(Effect, EffectContext, 1.f))
	{
		...
		TSharedPtr<FGameplayTag> DebufDamageType = MakeShareable(new FGameplayTag(DamageType));
		AuraContext->SetDamageType(DebufDamageType);
...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	...
	if (LocalIncomingDamage > 0.f)
	{
		...
		else
		{
			...
			if (!KnockbackForce.IsNearlyZero(5.f))
			{
		...
		if (UAuraAbilitySystemLibrary::IsSuccessfullDebuf(Props.EffectContextHandle))
		{
			Debuff(Props);
		}
	}
